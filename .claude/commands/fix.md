...
`resources/js/**`) without a spec change, so resolve the spec side first.
...
3. Run `./vendor/bin/pint && php artisan config:clear && php artisan test`. Keep the
   spec's other scenarios green.
