...
- **Frontend**: 22 Alpine modules in `resources/js/alpine/` registered in `resources/js/app.js`. Shared mixins/helpers (not top-level): `upload-metadata` (batch metadata form), `thumbnail-generator` (client-side PDF/video thumbs), `tag-input-core` (`parseTagNames` + `tagInputCore` — comma/paste splitting shared by all four tag inputs: asset edit, upload metadata, grid bulk bar, grid row), `demo-geometry` (guided-demo spotlight geometry). Asset grid markup is `resources/views/assets/partials/grid.blade.php`, shared between index and embed.
