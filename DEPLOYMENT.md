...
PASSKEYS_RELYING_PARTY_ID=     # Relying-party host, e.g. orca.example.com (legacy WEBAUTHN_ID still honored)
PASSKEYS_ORIGINS=              # Optional comma-separated extra origins (legacy WEBAUTHN_ORIGINS still honored)
# Passkeys require HTTPS (or localhost) — make sure SSL is in place.
...
- [ ] Set `PASSKEYS_RELYING_PARTY_ID` if `APP_URL` host doesn't match the browser-facing host (e.g. proxy / multi-domain setups)
