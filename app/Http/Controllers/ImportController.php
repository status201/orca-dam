    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            DB::transaction(function () use ($asset, $row, &$updateData, &$userTagIds, &$refTagIds, $request) {
                ...
                // Handle user_tags (comma-separated)
                $tagNames = TagInputParser::parse($row['user_tags'] ?? null);
                if (! empty($tagNames)) {
                    $userTagIds = Tag::resolveUserTagIds($tagNames);
                    $asset->syncTagsWithAttribution($userTagIds, 'user');
                }

                // Handle reference_tags (comma-separated)
                $refTagNames = TagInputParser::parse($row['reference_tags'] ?? null);
