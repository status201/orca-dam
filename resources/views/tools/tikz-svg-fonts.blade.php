...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="tikzSvgFonts()">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span class="text-gray-700">{{ __('TikZ to SVG (Embedded Fonts)') }}</span>
        </div>
    </div>

    <div class="mb-6">
        <div class="flex items-center gap-3">
            <h1 class="text-3xl font-bold">{{ __('TikZ to SVG (Embedded Fonts)') }}</h1>
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                {{ __('Beta') }}
            </span>
        </div>
        <p class="text-gray-600 mt-1">{{ __('Render TikZ diagrams to SVG with base64-embedded fonts for portable, self-contained output.') }}</p>
    </div>
    ...
    {{-- Hidden iframe for TikZJax rendering (allow-same-origin needed for font fetching) --}}
    <iframe id="tikz-fonts-iframe" style="display:none" sandbox="allow-scripts allow-same-origin"></iframe>
    ...
    <div class="mt-8 bg-amber-50 border border-amber-200 rounded-lg overflow-hidden">
        <button
            ...
            <span class="font-semibold text-amber-900 text-sm">{{ __('TikZJax Technical Notes & Font Embedding') }}</span>
        ...
        <div class="hidden border-t border-amber-200 px-5 py-4 text-sm text-amber-900 space-y-4">
            ...
            <div>
                <h4 class="font-semibold text-amber-800 mb-1"><i class="attention fas fa-font mr-1.5 text-amber-500"></i>{{ __('Font embedding') }}</h4>
                <p class="text-amber-800">
                    {{ __('Unlike the standard TikZ to SVG tool, this version embeds the TeX fonts (BaKoMa TTF) directly into each SVG as base64-encoded') }}
                    <code class="bg-amber-100 px-1.5 py-0.5 rounded text-xs font-mono">@font-face</code>
                    {{ __('rules. This makes each SVG fully self-contained — it will render correctly everywhere, including in') }}
                    <code class="bg-amber-100 px-1.5 py-0.5 rounded text-xs font-mono">&lt;img&gt;</code>
                    {{ __('tags, design tools, and when served from S3 without external font dependencies.') }}
                </p>
            </div>

            <div>
                <h4 class="font-semibold text-amber-800 mb-1"><i class="attention fas fa-weight-hanging mr-1.5 text-amber-500"></i>{{ __('File size') }}</h4>
                <p class="text-amber-800">
                    {{ __('Embedded fonts add roughly 40-100 KB per font used (base64 encoded). A typical diagram uses 3-6 fonts, adding ~200-400 KB to the SVG. Diagrams without text (pure geometry) have no fonts embedded and remain small.') }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-amber-800">
                    ...
                    {{ __('These are harmless — the original TikZJax virtual filesystem only ships 4 files and does not include AMS font definitions. Standard TikZ diagrams render correctly despite these warnings.') }}
                </p>
            </div>

            <div class="pt-2 border-t border-amber-200 text-xs text-amber-700 flex flex-wrap gap-x-4 gap-y-1">
                <a href="https://github.com/kisonecat/tikzjax" target="_blank" rel="noopener" class="inline-flex items-center gap-1 hover:text-amber-900"><i class="fab fa-github"></i> kisonecat/tikzjax</a>
