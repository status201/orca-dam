...
106 tests across 20 spec files (101 literal `test()` calls plus 5 generated by the
`tools.spec.js` loop), on top of the 4 in `global.setup.js` — so
`npx playwright test --list` reports **110 tests in 21 files**. Counting only literal
...
upload-metadata · dashboard-tour · passkeys · asset-replace · discover(S3)
```
