...
export function SettingsApp() {
	...
	useEffect( () => {
		if (
			...
			setHealthStatus( {
				type: 'success',
				text: __( 'Settings saved.', 'orca-dam-picker' ),
			} );
		}
	}, [] );
		...
		try {
			...
			setHealthStatus(
				body.reachable
					? {
							type: 'success',
							text: __( 'Connected to ORCA.', 'orca-dam-picker' ),
					  }
					: {
							type: 'error',
							text:
								__( 'ORCA returned', 'orca-dam-picker' ) +
								' ' +
								body.orca_status,
					  }
			);
	...
	return (
		<VStack spacing={ 4 }>
				...
				<CardBody>
					<form method="POST" action={ config.saveUrl }>
						<input
							type="hidden"
							name="action"
							value="orca_dam_save_settings"
						/>
						<input
							type="hidden"
							name="_wpnonce"
							value={ config.saveNonce }
						/>
						<VStack spacing={ 3 }>
							<TextControl
								label={ __(
							...
							<TextControl
								...
								help={
									config.hasToken
										...
										: __(
												'Paste a token created in ORCA → API Tokens.',
												'orca-dam-picker'
										  )
								}
								value={ token }
							...
							<TextControl
								label={ __(
									'Default folder filter (optional)',
									'orca-dam-picker'
								) }
								value={ folder }
								onChange={ setFolder }
			...
			<Card>
				<CardHeader>
					<h2 style={ { margin: 0 } }>
						{ __( 'Usage tracking', 'orca-dam-picker' ) }
					</h2>
				</CardHeader>
				<CardBody>
					<p>
						{ __(
							'When this site uses an ORCA asset in a post, ORCA records a reference tag of the form',
							'orca-dam-picker'
						) }{ ' ' }
...
function BrokenAssetsCard( { restUrl, nonce } ) {
	...
	const load = () => {
		fetch( `${ restUrl }/broken`, { headers: { 'X-WP-Nonce': nonce } } )
			.then( ( r ) => r.json() )
			.then( ( body ) =>
				setState( { count: body.count ?? 0, items: body.items ?? [] } )
			)
	...
	return (
		<Card>
			<CardHeader>
				<h2 style={ { margin: 0 } }>
					{ __( 'Broken assets', 'orca-dam-picker' ) }
				</h2>
			</CardHeader>
			<CardBody>
				...
				{ state.count > 0 && (
					<>
						<p>
							{
								/* translators: %d: number of post shells referencing missing ORCA assets */
								__(
									'%d shell(s) reference assets that no longer exist in ORCA.',
									'orca-dam-picker'
								).replace( '%d', state.count )
							}
						</p>
						<ul
							style={ {
